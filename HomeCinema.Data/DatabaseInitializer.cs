...
{
    public class DatabaseInitializer :  CreateDatabaseIfNotExists<HomeCinemaContext>
    {
        ...
        {
            var context = new HomeCinemaContext();
            var generateGenres = GenerateGenres();

            foreach (var item in generateGenres)
            {
            ...
            var generateMovies = GenerateMovies();
            foreach (var item in generateMovies)
            {
                context.MovieSet.Add(item);
            }

            // create stocks
            var generateStocks = GenerateStocks();
            foreach (var item in generateStocks)
            {
                context.StockSet.Add(item);
            }

            // create customers
            var generateCustomers = GenerateCustomers();
            foreach (var item in generateCustomers)
            {
                context.CustomerSet.Add(item);
            }

            //  context.CustomerSet.AddOrUpdate(GenerateCustomers());

            // create roles
            var generateRoles = GenerateRoles();
            foreach (var item in generateRoles)
            {
                context.RoleSet.Add(item);
            }
